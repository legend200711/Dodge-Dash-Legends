...
window.addEventListener('beforeinstallprompt', e => {
  ...
  if (banner) { banner.style.display = 'flex'; banner.style.animation = 'bannerSlideUp 0.4s ease-out'; }
});
...
window.addEventListener('appinstalled', () => {
  ...
  showToast('✅ App installed! Launch from your home screen.', 3000);
});
...
function installApp() {
  ...
  _deferredInstallPrompt.userChoice.then(choice => {
    if (choice.outcome === 'accepted') { const banner=document.getElementById('install-banner'); if(banner) banner.style.display='none'; }
    _deferredInstallPrompt = null;
  });
}
